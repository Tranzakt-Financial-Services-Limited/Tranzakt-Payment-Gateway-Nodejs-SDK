...
describe("Tranzakt", () => {
  ...
  const mockStaticInvoiceData: CreateStaticInvoiceProps = {
    collectionId: "12345",
    title: "Test Invoice",
    payerName: "John Doe",
  ...
  it("should create a static invoice successfully", async () => {
    ...
    const response = await tranzakt.createStaticInvoice(mockStaticInvoiceData);
  ...
  it("should handle a 400 Bad Request error when creating a static invoice", async () => {
    ...
    await expect(
      tranzakt.createStaticInvoice(mockStaticInvoiceData)
